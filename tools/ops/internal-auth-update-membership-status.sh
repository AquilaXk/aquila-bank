...
if [[ $# -ne 8 ]]; then
  cat <<'USAGE' >&2
usage: tools/ops/internal-auth-update-membership-status.sh <base_url> <bootstrap_token> <actor_subject> <request_id> <user_id> <account_id> <membership_status> <reason>
...
# 운영 추적 기준: membership 회수도 user 상태 변경과 같은 헤더 세트를 강제합니다.
curl --fail-with-body --silent --show-error \
  ...
  --data "{\"membershipStatus\":\"${membership_status}\",\"reason\":\"${reason}\"}" \
