...
grep -F "base_url:" "${workflow}" >/dev/null
...
grep -F "bash tools/test/check-oci-real-multisource-public-evidence-autogen.sh" "${workflow}" >/dev/null
...
grep -F 'OCI_REAL_MULTISOURCE_BASE_URL="${BASE_URL_INPUT:-${OCI_REAL_MULTISOURCE_BASE_URL:-${STAGING_BASE_URL:-${K6_BASE_URL:-${OCI_REAL_MULTISOURCE_BASE_URL_VAR:-}}}}}"' "${workflow}" >/dev/null
grep -F 'OCI_REAL_MULTISOURCE_EVIDENCE_READY=false' "${workflow}" >/dev/null
grep -F "name: Generate real multisource public evidence artifacts" "${workflow}" >/dev/null
grep -F "if: env.OCI_REAL_MULTISOURCE_EVIDENCE_READY != 'true'" "${workflow}" >/dev/null
grep -F "OCI_REAL_MULTISOURCE_AUTOGEN_MODE: live" "${workflow}" >/dev/null
grep -F 'generated_env="${OCI_REAL_MULTISOURCE_OUTPUT_DIR}/${OCI_REAL_MULTISOURCE_NAME}-generated-evidence.env"' "${workflow}" >/dev/null
grep -F 'bash tools/test/run-oci-real-multisource-public-evidence-autogen.sh' "${workflow}" >/dev/null
grep -F 'source "${generated_env}"' "${workflow}" >/dev/null
grep -F "name: Validate real multisource evidence env" "${workflow}" >/dev/null
