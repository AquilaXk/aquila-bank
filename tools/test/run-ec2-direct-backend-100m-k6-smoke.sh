...
usage() {
  cat <<'USAGE' >&2
usage: tools/test/run-ec2-direct-backend-100m-k6-smoke.sh [--print-plan|--dry-run]

Environment:
  EC2_LOCAL_DB_CAPACITY_ENV_FILE optional env file
  EC2_DIRECT_BACKEND_BASE_URL required through env doctor
...
base_url="${EC2_DIRECT_BACKEND_BASE_URL:-}"
report_name="${EC2_DIRECT_K6_REPORT_NAME:-${run_id}-direct-backend}"

print_plan() {
  EC2_LOCAL_DB_TUNNEL_CHECK="${EC2_LOCAL_DB_TUNNEL_CHECK:-false}" \
    tools/test/run-ec2-local-db-capacity-env-doctor.sh --print-plan
  echo "[ec2-direct-k6] mode=${mode}"
  echo "[ec2-direct-k6] run_id=${run_id}"
  echo "[ec2-direct-k6] base_url=${base_url}"
  echo "[ec2-direct-k6] report_name=${report_name}"
  echo "[ec2-direct-k6] scenario=${EC2_K6_SCENARIO_MODE:-constant-vus}"
  echo "[ec2-direct-k6] vus=${EC2_K6_VUS:-8} duration=${EC2_K6_DURATION:-1m}"
}
