...
hikari-lifetime	run-hikari-001	30	1	oci://run-hikari/k6.json	oci://run-hikari/nginx.jsonl	oci://run-hikari/spring.json	oci://run-hikari/hikari.log	oci://run-hikari/postgres.tsv	n/a	n/a	oci://run-hikari/timeline.json	0.01	0	0	0	0	0	0	420	n/a	n/a	n/a	n/a	n/a	n/a	0	n/a	0	n/a	80	210	610	0	0	14.1	oci://run-hikari/hikari-config.tsv	120000	30000	300000	350000	oci://run-hikari/hikari-zero-warning.md	n/a	0	n/a
real-ip-multisource	run-realip-001	2	3	oci://run-realip/k6.json	oci://run-realip/nginx.jsonl	oci://run-realip/spring.json	oci://run-realip/hikari.log	oci://run-realip/postgres.tsv	n/a	n/a	oci://run-realip/timeline.json	0.07	0	0	0	0	0	0	430	n/a	n/a	n/a	n/a	n/a	n/a	0	n/a	0	n/a	82	215	620	0	0	15.2	n/a	0	0	0	0	n/a	n/a	0	n/a
mixed-workload-30m	run-mixed-001	30	1	oci://run-mixed/k6.json	oci://run-mixed/nginx.jsonl	oci://run-mixed/spring.json	oci://run-mixed/hikari.log	oci://run-mixed/postgres.tsv	n/a	n/a	oci://run-mixed/timeline.json	0.08	0	0	0	0	0	0	450	n/a	n/a	n/a	oci://run-mixed/workload-mix.json	oci://run-mixed/workload-components.tsv	oci://run-mixed/outbox-lag.tsv	0	n/a	0	n/a	85	225	630	1	0	16.3	n/a	0	0	0	0	n/a	read,write,auth,notification,sse	440	oci://run-mixed/read-429-source.tsv
cold-warm-cache	run-cache-001	3	1	oci://run-cache/k6.json	oci://run-cache/nginx.jsonl	oci://run-cache/spring.json	oci://run-cache/hikari.log	oci://run-cache/postgres.tsv	n/a	oci://run-cache/cache.json	oci://run-cache/timeline.json	0.02	0	0	0	0	0	0	410	n/a	n/a	n/a	n/a	n/a	n/a	0	n/a	0	n/a	78	205	600	0	0	13.8	n/a	0	0	0	0	n/a	n/a	0	n/a
...
p999-long-correlation	run-p999-001	30	1	oci://run-p999/k6.json	oci://run-p999/nginx.jsonl	oci://run-p999/spring.json	oci://run-p999/hikari.log	oci://run-p999/postgres.tsv	n/a	n/a	oci://run-p999/timeline.json	0.06	0	0	0	0	0	0	490	oci://run-p999/checkpoint.tsv	oci://run-p999/temp-files.tsv	oci://run-p999/nginx-upstream.tsv	n/a	n/a	n/a	0	n/a	0	n/a	95	220	650	3	0	18.5	n/a	0	0	0	0	n/a	n/a	0	n/a
TSV
...
grep -F $'\thikari_config_ref\thikari_max_lifetime_ms\thikari_keepalive_time_ms\tpostgres_idle_timeout_ms\toci_nat_idle_timeout_ms\thikari_zero_warning_soak_ref' "${execution_tsv}" >/dev/null
grep -F $'\tworkload_components\tread_p999_ms\tread_429_source_ref' "${execution_tsv}" >/dev/null
awk -F '\t' '$1 == "p999-long-correlation" && $33 == 95 && $34 == 220 && $35 == 650 && $36 == 3 && $37 == 0 && $38 == 18.5 { found = 1 } END { exit !found }' "${execution_tsv}"
awk -F '\t' '$1 == "hikari-lifetime" && $4 == 30 && $39 == "oci://run-hikari/hikari-config.tsv" && $40 == 120000 && $41 == 30000 && $42 == 300000 && $43 == 350000 && $44 == "oci://run-hikari/hikari-zero-warning.md" { found = 1 } END { exit !found }' "${execution_tsv}"
awk -F '\t' '$1 == "mixed-workload-30m" && $45 == "read,write,auth,notification,sse" && $46 == 440 && $47 == "oci://run-mixed/read-429-source.tsv" { found = 1 } END { exit !found }' "${execution_tsv}"
