...
cat >"${input_tsv}" <<'TSV'
scenario	run_id	duration_min	source_ips	k6_summary_ref	nginx_access_ref	spring_metrics_ref	hikari_log_ref	postgres_wait_ref	deploy_event_ref	cache_state_ref	timeline_ref	edge_429_rate	backend_429_count	five_xx_count	nginx_499_count	hikari_validation_warnings	db_pool_pending_max	p999_ms
hikari-lifetime	run-hikari-001	10	1	oci://run-hikari/k6.json	oci://run-hikari/nginx.jsonl	oci://run-hikari/spring.json	oci://run-hikari/hikari.log	oci://run-hikari/postgres.tsv	n/a	n/a	oci://run-hikari/timeline.json	0.01	0	0	0	0	0	420
real-ip-multisource	run-realip-001	2	3	oci://run-realip/k6.json	oci://run-realip/nginx.jsonl	oci://run-realip/spring.json	oci://run-realip/hikari.log	oci://run-realip/postgres.tsv	n/a	n/a	oci://run-realip/timeline.json	0.07	0	0	0	0	0	430
mixed-workload-30m	run-mixed-001	30	1	oci://run-mixed/k6.json	oci://run-mixed/nginx.jsonl	oci://run-mixed/spring.json	oci://run-mixed/hikari.log	oci://run-mixed/postgres.tsv	n/a	n/a	oci://run-mixed/timeline.json	0.08	0	0	0	0	0	450
cold-warm-cache	run-cache-001	3	1	oci://run-cache/k6.json	oci://run-cache/nginx.jsonl	oci://run-cache/spring.json	oci://run-cache/hikari.log	oci://run-cache/postgres.tsv	n/a	oci://run-cache/cache.json	oci://run-cache/timeline.json	0.02	0	0	0	0	0	410
deploy-drain	run-drain-001	5	1	oci://run-drain/k6.json	oci://run-drain/nginx.jsonl	oci://run-drain/spring.json	oci://run-drain/hikari.log	oci://run-drain/postgres.tsv	oci://run-drain/deploy.json	n/a	oci://run-drain/timeline.json	0.04	0	0	0	0	0	470
p999-long-correlation	run-p999-001	30	1	oci://run-p999/k6.json	oci://run-p999/nginx.jsonl	oci://run-p999/spring.json	oci://run-p999/hikari.log	oci://run-p999/postgres.tsv	n/a	n/a	oci://run-p999/timeline.json	0.06	0	0	0	0	0	490
TSV
