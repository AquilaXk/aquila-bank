...
cat >"${fail_json}" <<'JSON'
{
  "metrics": {
    ...
    "aquila_transaction_edge_429_rate": {"values": {"rate": 0.101}},
    "aquila_transaction_edge_429_count": {"values": {"count": 101}},
    "aquila_transaction_backend_429_rate": {"values": {"rate": 0.019}},
    "aquila_transaction_backend_429_count": {"values": {"count": 19}},
    "aquila_transaction_unknown_429_rate": {"values": {"rate": 0}},
    "aquila_transaction_unknown_429_count": {"values": {"count": 0}},
    ...
    "aquila_transaction_accepted_200_rate": {"values": {"rate": 0.88}},
    "aquila_transaction_accepted_200_count": {"values": {"count": 880}}
  }
}
JSON
...
vu16	200	PASSED	200	none	none	none	none	880	0	0	10.000	10.000
vu16	429	PASSED	429	backend	backend-admission	backend-admission	backend-admission	6	0	6	40.000	40.000
vu16	429	PASSED	429	backend	fairness-limiter	fairness-limiter	fairness-limiter	13	0	13	45.000	45.000
vu16	429	REJECTED	none	nginx-edge	edge-rate-limit	none	edge-rate-limit	101	0	101	0.000	0.000
TSV
...
grep -F "| backend 429 | pass | 0.05 | 50 |" "${report_md}" >/dev/null
...
saturation_output="$(
  ...
  SOURCE_429_BACKEND_FAIL_COUNT=25 \
  SOURCE_429_BACKEND_ADMISSION_FAIL_COUNT=10 \
  SOURCE_429_FAIRNESS_FAIL_COUNT=20 \
...
grep -F $'edge\tpass\t0.101\t101\t1' "${saturation_summary_tsv}" >/dev/null
grep -F $'backend\tpass\t0.019\t19\t0.02,count<=25' "${saturation_summary_tsv}" >/dev/null
grep -F $'backend-admission\tpass\t0.006000\t6\t0.02,count<=10' "${saturation_summary_tsv}" >/dev/null
grep -F $'fairness-limiter\tpass\t0.013000\t13\t0.02,count<=20' "${saturation_summary_tsv}" >/dev/null
...
grep -F "| backend-admission | pass | 0.006000 | 6 | 0.02,count<=10 |" "${saturation_report_md}" >/dev/null
grep -F "| fairness-limiter | pass | 0.013000 | 13 | 0.02,count<=20 |" "${saturation_report_md}" >/dev/null

echo "[transaction-read-429-source] saturation source count budget fails"
count_fail_nginx_aggregate_tsv="${temp_dir}/nginx-aggregate-count-fail.tsv"
awk -F '\t' 'BEGIN { OFS = FS } NR == 1 { print; next } $6 == "fairness-limiter" { $9 = 21; $11 = 21 } $2 == "200" { $9 = 872 } { print }' \
  "${nginx_aggregate_tsv}" >"${count_fail_nginx_aggregate_tsv}"
if SOURCE_429_GATE_NAME=source-vu16-count-fail \
  SOURCE_429_SUMMARY_JSON="${saturation_json}" \
  SOURCE_429_OUTPUT_DIR="${output_dir}" \
  SOURCE_429_RUN_ID=vu16 \
  SOURCE_429_GATE_MODE=saturation-observation \
  ...
  SOURCE_429_BACKEND_ADMISSION_FAIL_COUNT=10 \
  SOURCE_429_FAIRNESS_FAIL_COUNT=20 \
  SOURCE_429_NGINX_AGGREGATE_TSV="${count_fail_nginx_aggregate_tsv}" \
    "${runner}" >/dev/null 2>&1; then
  echo "source gate unexpectedly passed saturation fairness count over budget" >&2
  exit 1
fi
...
grep -F "SOURCE_429_BACKEND_ADMISSION_FAIL_COUNT" "${runner}" >/dev/null
grep -F "SOURCE_429_FAIRNESS_FAIL_COUNT" "${runner}" >/dev/null
...
if SOURCE_429_BACKEND_ADMISSION_FAIL_COUNT=-1 "${runner}" --print-plan >/dev/null 2>&1; then
  echo "invalid SOURCE_429_BACKEND_ADMISSION_FAIL_COUNT unexpectedly succeeded" >&2
  exit 1
fi
