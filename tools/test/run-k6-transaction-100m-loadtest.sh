...
Optional environment:
  ...
  K6_BACKEND_429_RATE_THRESHOLD
                       max backend 429 rate in overload mode, default 0.005
  ...
  K6_CONSTANT_VUS_GATE_ROLE constant-vus gate role, default not-selected
  ...
  K6_REMOTE_PROMETHEUS_RW_REQUIRED
                       fail when remote-write is unreachable, default true
...
K6_BACKEND_429_RATE_THRESHOLD="${K6_BACKEND_429_RATE_THRESHOLD:-0.005}"
...
K6_CONSTANT_VUS_GATE_ROLE="${K6_CONSTANT_VUS_GATE_ROLE:-not-selected}"
...
K6_REMOTE_PROMETHEUS_RW_REQUIRED="${K6_REMOTE_PROMETHEUS_RW_REQUIRED:-true}"
...
require_bool_value K6_REMOTE_PROMETHEUS_RW_REQUIRED
...
require_rate K6_BACKEND_429_RATE_THRESHOLD
...
print_plan() {
  ...
  echo "[k6-transaction-100m] constant-vus gate role=${K6_CONSTANT_VUS_GATE_ROLE}"
  ...
  echo "[k6-transaction-100m] backend 429 rate threshold=${K6_BACKEND_429_RATE_THRESHOLD}"
  ...
  if [[ "${K6_GENERATOR_MODE}" == "docker-context" ]]; then
    ...
    if [[ "${K6_OBSERVABILITY_MODE}" == "prometheus" ]]; then
      ...
      echo "[k6-transaction-100m] remote prometheus required=${K6_REMOTE_PROMETHEUS_RW_REQUIRED}"
    else
...
write_run_context() {
  {
    ...
    echo "K6_CONSTANT_VUS_GATE_ROLE=${K6_CONSTANT_VUS_GATE_ROLE}"
...
remote_prometheus_remote_write_preflight() {
  ...
  if ! remote_preflight_status_ok "${status}"; then
    ...
    if [[ "${K6_REMOTE_PROMETHEUS_RW_REQUIRED}" == "false" ]]; then
      echo "[k6-transaction-100m] remote prometheus remote-write optional; downgrading observability to summary-only status=${status}" >&2
      K6_OBSERVABILITY_MODE="summary-only"
      export K6_OBSERVABILITY_MODE
      return 0
    fi
...
run_k6_local() {
  ...
  docker compose "${compose_files[@]}" --profile loadtest run "${run_args[@]}" \
    ...
    -e K6_BACKEND_429_RATE_THRESHOLD="${K6_BACKEND_429_RATE_THRESHOLD}" \
    ...
    -e K6_CONSTANT_VUS_GATE_ROLE="${K6_CONSTANT_VUS_GATE_ROLE}" \
