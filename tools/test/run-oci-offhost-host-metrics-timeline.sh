...
required_columns="run_id,phase,host_role,host_name,host_id,vm_id,network_id,docker_context,sample_started_at_utc,sample_ended_at_utc,sample_count,cpu_pct_avg,cpu_pct_max,rx_mbps_avg,rx_mbps_max,tx_mbps_avg,tx_mbps_max,artifact_uri,summary_ref,artifact_pack_uri"
...
awk -F '\t' -v OFS='\t' -v expected_run_id="${run_id}" -v required_phases="${required_phases}" '
  ...
  NR == 1 {
    ...
    split("run_id phase host_role host_name host_id vm_id network_id docker_context sample_started_at_utc sample_ended_at_utc sample_count cpu_pct_avg cpu_pct_max rx_mbps_avg rx_mbps_max tx_mbps_avg tx_mbps_max artifact_uri summary_ref artifact_pack_uri", required, " ")
    for (i in required) {
...
timeline_table="$(awk -F '\t' '
  BEGIN {
    print "| Phase | Role | Host | VM id | Network id | CPU avg % | CPU max % | RX max Mbps | TX max Mbps | Summary |"
    print "| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- |"
  }
  NR > 1 {
    printf "| %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n", $2, $3, $4, $6, $7, $12, $13, $15, $17, $19
  }
' "${timeline_tsv}")"
