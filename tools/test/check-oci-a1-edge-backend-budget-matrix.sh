...
grep -F "edge_transaction_read_rate_rps=48" <<<"${plan}" >/dev/null
...
grep -F "| edge transaction-read rate | 48r/s |" "${report_md}" >/dev/null
...
grep -F 'limit_req_zone $binary_remote_addr zone=aquila_bank_transaction_read_per_ip:10m rate=48r/s;' ops/nginx/nginx.conf >/dev/null
grep -F 'limit_req zone=aquila_bank_transaction_read_per_ip burst=24 delay=8;' ops/nginx/nginx.conf >/dev/null
