...
offhost-run-20260504	arrival16	generator	k6-generator-a	ocid1.instance.oc1..generatora	vm-k6-a	subnet-generator-a	oci-k6-generator-a	2026-05-04T01:00:00Z	2026-05-04T01:01:00Z	12	load-coupled	5	31.2	44.1	10.5	18.2	12.1	19.7	oci://aquila-evidence/transaction-read/offhost-run-20260504/arrival16/generator.tsv	build/reports/k6/offhost-run-20260504/arrival16-summary.json	oci://aquila-evidence/transaction-read/offhost-run-20260504
offhost-run-20260504	arrival16	target	oci-a1-staging	ocid1.instance.oc1..target	vm-target	subnet-target	target	2026-05-04T01:00:00Z	2026-05-04T01:01:00Z	12	load-coupled	5	52.4	66.8	21.1	30.4	25.6	34.2	oci://aquila-evidence/transaction-read/offhost-run-20260504/arrival16/target.tsv	build/reports/k6/offhost-run-20260504/arrival16-summary.json	oci://aquila-evidence/transaction-read/offhost-run-20260504
offhost-run-20260504	vu16	generator	k6-generator-a	ocid1.instance.oc1..generatora	vm-k6-a	subnet-generator-a	oci-k6-generator-a	2026-05-04T01:05:00Z	2026-05-04T01:06:00Z	12	load-coupled	5	34.8	49.0	12.0	20.0	15.1	23.4	oci://aquila-evidence/transaction-read/offhost-run-20260504/vu16/generator.tsv	build/reports/k6/offhost-run-20260504/vu16-summary.json	oci://aquila-evidence/transaction-read/offhost-run-20260504
offhost-run-20260504	vu16	target	oci-a1-staging	ocid1.instance.oc1..target	vm-target	subnet-target	target	2026-05-04T01:05:00Z	2026-05-04T01:06:00Z	12	load-coupled	5	58.2	70.3	24.2	34.0	28.0	38.2	oci://aquila-evidence/transaction-read/offhost-run-20260504/vu16/target.tsv	build/reports/k6/offhost-run-20260504/vu16-summary.json	oci://aquila-evidence/transaction-read/offhost-run-20260504
offhost-run-20260504	burst-matrix	generator	k6-generator-a	ocid1.instance.oc1..generatora	vm-k6-a	subnet-generator-a	oci-k6-generator-a	2026-05-04T01:10:00Z	2026-05-04T01:15:00Z	60	load-coupled	5	43.1	59.3	18.7	31.2	22.4	36.8	oci://aquila-evidence/transaction-read/offhost-run-20260504/burst-matrix/generator.tsv	build/reports/k6/offhost-run-20260504/burst-matrix.tsv	oci://aquila-evidence/transaction-read/offhost-run-20260504
offhost-run-20260504	burst-matrix	target	oci-a1-staging	ocid1.instance.oc1..target	vm-target	subnet-target	target	2026-05-04T01:10:00Z	2026-05-04T01:15:00Z	60	load-coupled	5	63.7	76.2	35.4	51.8	40.0	59.0	oci://aquila-evidence/transaction-read/offhost-run-20260504/burst-matrix/target.tsv	build/reports/k6/offhost-run-20260504/burst-matrix.tsv	oci://aquila-evidence/transaction-read/offhost-run-20260504
TSV
...
grep -F "max_sample_interval_seconds=5" <<<"${plan}" >/dev/null
grep -F "require_load_coupled=true" <<<"${plan}" >/dev/null
...
grep -F "load-coupled samples: required" "${report_md}" >/dev/null
grep -F "max sample interval seconds: 5" "${report_md}" >/dev/null
grep -F "artifact pack URI: oci://aquila-evidence/transaction-read/offhost-run-20260504" "${report_md}" >/dev/null
grep -F $'run_id\tphase\thost_role\thost_name\thost_id\tvm_id\tnetwork_id\tdocker_context\tsample_started_at_utc\tsample_ended_at_utc\tsample_count\tsample_source\tsample_interval_seconds' "${timeline_tsv}" >/dev/null
jq -e '.summary.gate_status == "pass" and .summary.phase_count == 3 and (.items | length == 6)' "${timeline_json}" >/dev/null
jq -e '.items[] | select(.phase == "burst-matrix" and .host_role == "target" and .sample_source == "load-coupled" and .sample_interval_seconds == 5 and .cpu_pct_max == 76.2)' "${timeline_json}" >/dev/null
...
awk -F '\t' 'BEGIN { OFS = FS } NR == 1 { print; next } $2 == "arrival16" && $3 == "generator" { $21 = "n/a" } { print }' \
...
echo "[oci-offhost-host-metrics-timeline] slow interval fails"
awk -F '\t' 'BEGIN { OFS = FS } NR == 1 { print; next } $2 == "arrival16" && $3 == "generator" { $13 = "10" } { print }' \
  "${input_tsv}" >"${input_tsv}.slow"
if OCI_OFFHOST_HOST_METRICS_TIMELINE_NAME=offhost-timeline-slow \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_RUN_ID=offhost-run-20260504 \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_INPUT_TSV="${input_tsv}.slow" \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_REQUIRE_LOAD_COUPLED=true \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_OUTPUT_DIR="${temp_dir}/slow-output" \
    "${runner}" >"${temp_dir}/slow.log" 2>&1; then
  echo "off-host host metrics timeline unexpectedly passed slow sampler interval" >&2
  exit 1
fi
grep -F "load-coupled sampler interval must be <= 5 seconds" "${temp_dir}/slow.log" >/dev/null

echo "[oci-offhost-host-metrics-timeline] fallback cannot satisfy load-coupled gate"
awk -F '\t' 'BEGIN { OFS = FS } NR == 1 { print; next } { $12 = "fallback-snapshot"; $13 = "0" } { print }' \
  "${input_tsv}" >"${input_tsv}.fallback"
if OCI_OFFHOST_HOST_METRICS_TIMELINE_NAME=offhost-timeline-fallback-rejected \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_RUN_ID=offhost-run-20260504 \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_INPUT_TSV="${input_tsv}.fallback" \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_REQUIRE_LOAD_COUPLED=true \
  OCI_OFFHOST_HOST_METRICS_TIMELINE_OUTPUT_DIR="${temp_dir}/fallback-output" \
    "${runner}" >"${temp_dir}/fallback.log" 2>&1; then
  echo "off-host host metrics timeline unexpectedly accepted fallback as load-coupled evidence" >&2
  exit 1
fi
grep -F "load-coupled sampler source required" "${temp_dir}/fallback.log" >/dev/null
