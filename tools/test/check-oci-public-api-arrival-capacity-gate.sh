...
grep -F "rates=4,5,6,7,8,10" <<<"${plan}" >/dev/null
...
grep -F "arrival-10rps target: 429 < 0.10, edge delayed ratio < 0.25, 5xx = 0, accepted p95 < 350ms" "${report_md}" >/dev/null
grep -F "| 10 | pass | 0.080 | 0.040 | 0.040 | 0 | 0 | 0.12 | 330 |" "${report_md}" >/dev/null

echo "[oci-public-arrival-capacity] fail report"
write_summary 10 0.080 0.040 0.040 330 0 0.30
...
grep -F "4,5,6,7,8,10" "${runner}" >/dev/null
