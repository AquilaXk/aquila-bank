...
backend_429_status="$(status_for_rate "${backend_429_rate}" "${backend_fail_rate}")"
...
backend_admission_status="$(status_for_rate "${backend_admission_rate}" "${backend_fail_rate}")"
fairness_limiter_status="$(status_for_rate "${fairness_limiter_rate}" "${backend_fail_rate}")"
...

{
  ...
  printf "backend\t%s\t%s\t%s\t%s\n" "${backend_429_status}" "${backend_429_rate}" "${backend_429_count}" "${backend_fail_rate}"
  ...
  if [[ -n "${nginx_aggregate_tsv}" ]]; then
    printf "backend-admission\t%s\t%s\t%s\t%s\n" "${backend_admission_status}" "${backend_admission_rate}" "${backend_admission_count}" "${backend_fail_rate}"
    printf "fairness-limiter\t%s\t%s\t%s\t%s\n" "${fairness_limiter_status}" "${fairness_limiter_rate}" "${fairness_limiter_count}" "${backend_fail_rate}"
...
| --- | --- | ---: | ---: |
| backend-admission | ${backend_admission_status} | ${backend_admission_rate} | ${backend_admission_count} |
| fairness-limiter | ${fairness_limiter_status} | ${fairness_limiter_rate} | ${fairness_limiter_count} |
| nginx 499 | ${nginx_499_status} | ${nginx_499_rate} | ${nginx_499_count} |
| nginx 5xx | ${nginx_5xx_status} | ${nginx_5xx_rate} | ${nginx_5xx_count} |
REPORT
)"
fi
...
| 503 | ${transaction_503_status} | ${transaction_503_rate} | ${transaction_503_count} |
