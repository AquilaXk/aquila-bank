...
grep -F "telemetry-enabled=true" <<<"${plan}" >/dev/null
grep -F "reconnect-clients=3" <<<"${plan}" >/dev/null
grep -F "reconnect-rounds=3" <<<"${plan}" >/dev/null
...
grep -F "SSE_T3MICRO_TELEMETRY_ENABLED" "${script}" >/dev/null
grep -F "t3micro_start_container_telemetry" "${script}" >/dev/null
grep -F "SSE_RECONNECT_CLIENTS" tools/test/run-sse-reconnect-storm-smoke.sh >/dev/null
grep -F "archive_sse_result" "${script}" >/dev/null
grep -F "docs/performance-results" "${script}" >/dev/null

echo "[sse-reconnect-t3micro] concurrent matrix plan"
matrix_plan="$(
  SSE_RECONNECT_CLIENT_MATRIX=1,3 \
  SSE_RECONNECT_ROUNDS=2 \
    tools/test/run-sse-reconnect-concurrent-user-matrix.sh --print-plan
)"
grep -F "clients=1,3" <<<"${matrix_plan}" >/dev/null
grep -F "rounds=2" <<<"${matrix_plan}" >/dev/null
grep -F "runner=tools/test/run-sse-reconnect-storm-t3micro-gate.sh" <<<"${matrix_plan}" >/dev/null
...
grep -F "SSE_RECONNECT_CLIENTS=1" <<<"${matrix_dry_run}" >/dev/null
grep -F "SSE_RECONNECT_CLIENTS=3" <<<"${matrix_dry_run}" >/dev/null
grep -F "SSE_RECONNECT_ROUNDS=2" <<<"${matrix_dry_run}" >/dev/null
...
if SSE_T3MICRO_TELEMETRY_ENABLED=maybe "${script}" --print-plan >/dev/null 2>&1; then
  echo "SSE_T3MICRO_TELEMETRY_ENABLED=maybe unexpectedly succeeded" >&2
  exit 1
fi
if SSE_RECONNECT_CLIENT_MATRIX=0 tools/test/run-sse-reconnect-concurrent-user-matrix.sh --print-plan >/dev/null 2>&1; then
  echo "SSE_RECONNECT_CLIENT_MATRIX=0 unexpectedly succeeded" >&2
  exit 1
fi
