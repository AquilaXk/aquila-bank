...
grep -F "required_scenarios=hikari-10m,multisource-baseline,prometheus-30m,mixed-interference" <<<"${plan}" >/dev/null
grep -F "min_multisource_public_ips=2" <<<"${plan}" >/dev/null
...
grep -F "multi-source public baseline requires source_ips >= 2 and source_ip_breakdown_ref" "${report_md}" >/dev/null
...
grep -F $'mixed-interference\tpass\t30\t2\t0\t22\t0\t0\t345\tpass\tnone' "${summary_tsv}" >/dev/null
...
echo "[transaction-read-oci-capacity-evidence] multisource source IP fails"
awk -F '\t' 'BEGIN { OFS = FS } NR == 1 { print; next } $1 == "multisource-baseline" { $3 = "1" } { print }' \
  "${input_tsv}" >"${input_tsv}.source-fail"
if OCI_CAPACITY_EVIDENCE_NAME=oci-source-fail \
  OCI_CAPACITY_EVIDENCE_INPUT_TSV="${input_tsv}.source-fail" \
  OCI_CAPACITY_EVIDENCE_OUTPUT_DIR="${output_dir}" \
    "${runner}" >/dev/null 2>&1; then
  echo "OCI capacity evidence unexpectedly passed single source IP" >&2
  exit 1
fi
