...
- 선택 env:
  ...
  - `NGINX_TRANSACTION_READ_HOT_RATE_RPS` 기본값 `96`
  - `NGINX_TRANSACTION_READ_ARCHIVE_RATE_RPS` 기본값 `96`
  - `NGINX_TRANSACTION_READ_HOT_BURST` 기본값 `12`
  - `NGINX_TRANSACTION_READ_ARCHIVE_BURST` 기본값 `12`
  - `NGINX_TRANSACTION_READ_HOT_DELAY` 기본값 `1`, `0`이면 `nodelay`
  - `NGINX_TRANSACTION_READ_ARCHIVE_DELAY` 기본값 `1`, `0`이면 `nodelay`
...
- `limit_req_zone $binary_remote_addr zone=aquila_bank_transaction_hot_per_ip:10m rate=96r/s;`
- `limit_req_zone $binary_remote_addr zone=aquila_bank_transaction_archive_per_ip:10m rate=96r/s;`
...
- `location = /api/v1/transactions`에는 `limit_req zone=aquila_bank_transaction_hot_per_ip burst=12 delay=1;`를 적용합니다.
- `location = /api/v1/transactions/archive`에는 `limit_req zone=aquila_bank_transaction_archive_per_ip burst=12 delay=1;`를 적용합니다.
...
- transaction-read는 짧은 burst만 `delay=1` queue로 흡수하고, queue 초과는 429로 돌려 p95와 client backoff를 분리합니다.
