...
- 선택 env:
  ...
  - `NGINX_TRANSACTION_READ_HOT_RATE_RPS` 기본값 `256`
  - `NGINX_TRANSACTION_READ_ARCHIVE_RATE_RPS` 기본값 `256`
  - `NGINX_TRANSACTION_READ_HOT_BURST` 기본값 `256`
  - `NGINX_TRANSACTION_READ_ARCHIVE_BURST` 기본값 `256`
...
- `location = /api/v1/transfers`, `location ~ ^/api/v1/transfers/[^/]+/reversal$`에는 `limit_req zone=aquila_bank_transfer_per_ip burst=16 nodelay;`를 적용합니다.
...
- `burst80`: 기본 운영 후보. arrival-rate 16/s 정상 구간을 delay queue 없이 통과시키고 burst80에서 edge 429 `<= 10%`를 목표로 합니다.
- `burst64`: 이전 운영 후보. burst80 promotion이 흔들릴 때 rollback 비교 기준으로만 사용합니다.
