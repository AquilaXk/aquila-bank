...
resource "oci_core_security_list" "public" {
  ...
  dynamic "ingress_security_rules" {
    for_each = local.http_ingress_cidrs

    content {
      description = "Optional HTTP ingress for app smoke"
      protocol    = "6"
  ...
  dynamic "ingress_security_rules" {
    for_each = local.https_ingress_cidrs

    content {
      description = "Optional HTTPS ingress for app smoke"
      protocol    = "6"
      source      = ingress_security_rules.value
      source_type = "CIDR_BLOCK"
