...
대용량 트래픽을 작은 인프라에서 무제한 처리하는 대신, EC2 `t3.small` + RDS PostgreSQL `18.3` `db.t4g.medium` + gp3 150GiB 기준으로 과부하를 제한하고 1억 건 규모 거래 데이터를 bounded query로 조회하는 것을 목표로 하는 웹뱅킹 프로젝트입니다.
...
- 목표:
  ...
  - 제한된 인프라(EC2 `t3.small` + RDS PostgreSQL `18.3` `db.t4g.medium` + gp3 150GiB)에서도 운영 가능한 구조 지향
...
- deployed environment: `EC2 t3.small + RDS PostgreSQL 18.3 db.t4g.medium + gp3 150GiB`

## Environment Split

- 로컬 개발: `Docker Compose + PostgreSQL 18`
- Kafka 로컬 검증: `docker compose --profile kafka up`로 명시적으로 opt-in 합니다.
- 로컬 small-instance 근사 검증: `compose.t3micro.yml`과 `tools/test/run-docker-t3micro-capacity-smoke.sh`로 CPU/메모리 cgroup 제한을 적용합니다.
- 로컬 HTTP 부하 테스트: `compose.loadtest.yml`로 backend, k6, Prometheus, Grafana, Alertmanager, Postgres exporter를 함께 띄웁니다. Prometheus/Grafana는 부하테스트/선택 운영 자산이며 같은 app host 상시 필수 구성에서 제외합니다.
- 로컬 1억 건 synthetic 조회 테스트: dataset 생성은 `tools/test/prepare-transaction-read-model-100m-fixture.sh`, bounded 조회 부하는 `tools/test/run-transaction-read-model-100m-k6-local.sh --k6-only`로 분리합니다. 생성 phase는 별도 PostgreSQL budget을 사용하고, 조회 phase만 small-instance budget으로 제한합니다.
- 배포 환경: `EC2 t3.small + RDS PostgreSQL 18.3 db.t4g.medium + gp3 150GiB`
...
- Docker 근사 검증은 AWS `t3.small`의 CPU credit, gp3 지연, 실제 네트워크를 재현하지 못하므로 최종 120% headroom 판정은 EC2 `t3.small` + RDS `db.t4g.medium` staging smoke 결과로 닫습니다.
...
- 읽기 경로는 대용량 트래픽 방어, 1억 건 저장 규모, `t3.small` 운영 한계를 함께 고려해 경량화와 분리를 우선합니다.
